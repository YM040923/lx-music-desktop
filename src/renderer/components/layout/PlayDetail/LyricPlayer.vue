...
export default {
  ...
  setup() {
    ...
    const scrollSelectContentToActiveLine = () => {
      const container = dom_lrc_select_content.value
      if (!container) return
      const activeLine = container.children[lyric.line]
      if (!activeLine) return
      container.scrollTop = activeLine.offsetTop - (container.clientHeight * 0.38)
    }

    watch([isShowLrcSelectContent, () => lyric.line], ([isShow]) => {
      if (!isShow) return
      void nextTick(() => {
        requestAnimationFrame(scrollSelectContentToActiveLine)
      })
    })
...
.lyric {
  ...
  -webkit-mask-image: linear-gradient(transparent 0%, rgba(255, 255, 255, .82) 10%, #fff 24%, #fff 76%, rgba(255, 255, 255, .82) 90%, transparent 100%);
  cursor: grab;
  padding: 0 16px;
  ...
  :global {
    .font-lrc {
      color: rgba(31, 45, 58, .68);
      opacity: .94;
    }
    .line-content {
      line-height: 1.3;
      padding: calc(var(--playDetail-lrc-font-size, 16px) * 0.45) 1px;
      overflow-wrap: break-word;
      color: rgba(31, 45, 58, .64);
      transition: color 0.4s ease, font-size 0.4s ease;

      .extended { font-size: 0.75em; margin-top: 4px; }

      &.line-mode.active .font-lrc, &.font-mode.played .font-lrc {
        color: rgba(31, 45, 58, .78);
        opacity: .95;
      }

      &.line-mode.active {
        .font-lrc {
          color: rgba(28, 40, 52, .9);
          font-size: 1.15em;
          font-weight: 600;
          opacity: 1;
          text-shadow:
            0 1px 0 rgba(255, 255, 255, .65),
            0 12px 26px rgba(255, 255, 255, .26);
        }
      }
      ...
      &.font-mode > .line > .font-lrc {
        > span {
          ...
          background-color: rgba(31, 45, 58, .66);
          background-image: -webkit-linear-gradient(top, rgba(28, 40, 52, .9), rgba(28, 40, 52, .9));
...
.skip {
  ...
  // opacity: .5;
  .line {
    border-top: 2px dotted rgba(31, 47, 61, .36);
    opacity: .42;
    margin-right: 30px;
...
.lyricSelectContent {
  ...
  .lyricSelectlineExtended {
    font-size: 14px;
    color: rgba(39, 55, 68, .54);
  }
  .lrcActive {
    color: rgba(26, 39, 52, .92);
    font-weight: 600;
